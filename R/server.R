...
scDRcell <- function(inpConf, inpMeta, inpdrX, inpdrY, inp1, inpsub1, inpsub2, 
  ...
  if(inpdrX == "UMAPHarmony1") { 
    ...
    ggOut = ggOut + xlim(sc1def$xlim1,sc1def$xlim2)+ylim(sc1def$ylim1,sc1def$ylim2)
  }
...
shinyServer(function(input, output, session) { 
 ...
 optCrt="{ option_create: function(data,escape) {return('<div class=\"create\"><strong>' + '</strong></div>');} }" 
  ...
  scvolcano <- function(inpMarkers){
    ...
    minfdr = 10^-(1/3 *(-log10(min(inpMarkers[cluster == input$sc1de1inp]$p_val_adj))))
    ## minfdr = 0.05
    minfdr2 = 10^-(2/3 *(-log10(min(inpMarkers[cluster == input$sc1de1inp]$p_val_adj))))
    
    ggData$Significance = ifelse(ggData$p_val_adj < minfdr & abs(ggData$avg_log2FC) >= 0.58,
                                 ifelse(ggData$avg_log2FC > 0.0
                                        ,sc1def$treatment1,sc1def$treatment2)
                                 ,'Not siginficant'
    )
    ggData$Significance <- factor(ggData$Significance
                                  , levels = c(sc1def$treatment1,sc1def$treatment2,'Not siginficant'))
    ...
    ggOut =
      ...
      geom_text_repel(data = subset(ggData, p_val_adj < minfdr2 ),aes(label = gene))
    return(ggOut)
  }
...
output$sc1c2oup.png <- downloadHandler( 
  ...
  sclogo <- function(inplogos){
    # Prepare logo
    seqlogo = ggseqlogo(inplogos[c(grep(input$sc2logoinp, names(inplogos)))])
    return(seqlogo)
  }
  ...
  output$sc2logooup.pdf <- downloadHandler(
    ...
    content = function(file) { ggsave(
      file, device = "pdf", height = input$sc1logooup.h, width = input$sc2logooup.w, useDingbats = FALSE,
