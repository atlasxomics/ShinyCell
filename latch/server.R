...
seMarker_treatment <-  readRDS("markersGS_treatment.rds")
# for motif heatmap
seEnrich_cluster <-  readRDS("enrichMotifs_clusters.rds")
seEnrich_sample <-  readRDS("enrichMotifs_sample.rds")
seEnrich_treatment <-  readRDS("enrichMotifs_treatment.rds")
...
scDRcell <- function(inpConf, inpMeta, inpdrX, inpdrY, inp1, inpsub1, inpsub2, 
  ...
  # if(inpdrX == "UMAPHarmony1") {
  #   ggOut = ggOut
  # } else if (inpdrX == sc1def$dimred[3]) {
  #   ggOut = ggOut + xlim(sc1def$xlim11,sc1def$xlim12)+ylim(sc1def$ylim11,sc1def$ylim12)
  # } else if (inpdrX == sc1def$dimred[5]) {
  #   ggOut = ggOut + xlim(sc1def$xlim21,sc1def$xlim22)+ylim(sc1def$ylim21,sc1def$ylim22)
  # }  else  {
  #   ggOut = ggOut + xlim(sc1def$xlim31,sc1def$xlim32)+ylim(sc1def$ylim31,sc1def$ylim32)
  # }
  # return(ggOut)
...
scDRcoex <- function(inpConf, inpMeta, inpdrX, inpdrY, inp, 
  ...
  reduction <- substring(inpdrX, 1, nchar(inpdrX)-1)
...
scBubbHeat <- function(inpConf, inpMeta, inp, inpGrp, inpPlt, 
  ...
  if(inpGrp=="Clusters"){
    ...
    nClust <- ncol(mat)
    # 
    req_meta_data <- fread("./req_meta_data.csv")[,c(
      'V1'
      , 'Clusters'
      ,'Condition'
      ,"Sample")]
    ...
    out <- mapply(function(x,y) DataFrame(x,sampleID=rep(req_meta_data[which(req_meta_data$Clusters==y),]$V1,1,each=n2)), out,cluster)
    ...
    #  
      ...
      out <- mapply(function(x,y) DataFrame(x,sampleID=rep(req_meta_data[which(req_meta_data$Sample==y),]$V1,1,each=n2)), out,samples)
    ...
    for (i in seq_along(1:nTreatment)){
      ...
      n1[[i]] = nrow(req_meta_data[which(req_meta_data$Condition==Treatment[[i]]),])
    ...
    out <- mapply(function(x,y) DataFrame(x,sampleID=rep(req_meta_data[which(req_meta_data$Condition==y),]$V1,1,each=n2)), out,Treatment)


    out <- lapply(out, function(x) DataFrame(x,grpBy=rep("Condition",nrow(x))))
...
shinyServer(function(input, output, session) { 
 ...
 optCrt="{ option_create: function(data,escape) {return('<div class=\"create\"><strong>' + '</strong></div>');} }" 
  ...
  updateSelectizeInput(session, "sc1de1inp", choices = unique(inpMarkers$cluster), server = TRUE,
                       selected = unique(inpMarkers$cluster)[1], options = list(
                         maxOptions = 7, create = TRUE, persist = TRUE, render = I(optCrt)))
  ...
  output$sc1a1oup2 <- renderPlot({ 
    scDRgene(sc1conf, sc1meta, input$sc1a1drX2, input$sc1a1drY2, input$sc1a1inp2,  
             input$sc1a1sub1, input$sc1a1sub2, 
             "sc1gexpr.h5", sc1gene, 
  ...
  output$sc1a1oup2.pdf <- downloadHandler( 
    ...
    content = function(file) { ggsave( 
      ...
      plot = scDRgene(sc1conf, sc1meta, input$sc1a1drX2, input$sc1a1drY2, input$sc1a1inp2,  
                      input$sc1a1sub1, input$sc1a1sub2, 
                      "sc1gexpr.h5", sc1gene, 
                      input$sc1a1siz, input$sc1a1col2, input$sc1a1ord2, 
                      input$sc1a1fsz, input$sc1a1asp, input$sc1a1txt) ) 
  ...
  output$sc1a2oup2 <- renderPlot({ 
    scDRcell(sc1conf, sc1meta, input$sc1a2drX2, input$sc1a2drY2, input$sc1a2inp2,  
             input$sc1a2sub1, input$sc1a2sub2, 
  ...
  output$sc1a2oup2.pdf <- downloadHandler( 
    ...
    content = function(file) { ggsave( 
      ...
      plot = scDRcell(sc1conf, sc1meta, input$sc1a2drX2, input$sc1a2drY2, input$sc1a2inp2,   
                      input$sc1a2sub1, input$sc1a2sub2, 
                      input$sc1a2siz, input$sc1a2col2, input$sc1a2ord2,  
                      input$sc1a2fsz, input$sc1a2asp, input$sc1a2txt, input$sc1a2lab2) ) 
  }) 
  ...
  output$sc1a3oup2 <- renderPlot({ 
    scDRgene(sc1conf, sc1meta, input$sc1a3drX2, input$sc1a3drY2, input$sc1a3inp2,  
             input$sc1a3sub1, input$sc1a3sub2, 
             "sc1gexpr.h5", sc1gene, 
  ...
  output$sc1a3oup2.pdf <- downloadHandler( 
    ...
    content = function(file) { ggsave( 
      ...
      plot = scDRgene(sc1conf, sc1meta, input$sc1a3drX2, input$sc1a3drY2, input$sc1a3inp2,  
                      input$sc1a3sub1, input$sc1a3sub2, 
                      "sc1gexpr.h5", sc1gene, 
                      input$sc1a3siz, input$sc1a3col2, input$sc1a3ord2, 
                      input$sc1a3fsz, input$sc1a3asp, input$sc1a3txt) ) 
...
output$sc1c2oup.png <- downloadHandler( 
  ...
  grps <- c("Clusters","Sample","Condition")
  ...
  # observeEvent(input$range_max_1, {cat("You have chosen range_max_1:", input$range_max_1)})
  # observeEvent(input$range_min_1, {cat("You have chosen range_min_1:", input$range_min_1)})
  ...
  ##### START OF NEW CODE TO ADD ####################
  ### Tab1.x1: New tab for volcano
  inpMarkers_m = fread("inpMarkers_motif.txt")
  updateSelectizeInput(session, "sc2de1inp", choices = unique(inpMarkers_m$cluster), server = TRUE,
                       selected = unique(inpMarkers_m$cluster)[1], options = list(
                         maxOptions = 7, create = TRUE, persist = TRUE, render = I(optCrt)))
  #
  #
  scvolcano_m <- function(inpMarkers_m){
    ...
    ggData$Significance = ifelse(ggData$p_val_adj < minfdr,
                                 # & abs(ggData$avg_log2FC) >= 0.58,
                                 ifelse(ggData$avg_log2FC > 0.0
                                        ,sc2def$Condition1,sc2def$Condition2)
                                 ,'Not siginficant'
    )
    ggData$Significance <- factor(ggData$Significance
                                  , levels = c(sc2def$Condition1,sc2def$Condition2,'Not siginficant'))
  ...
  output$sc2a3oup2 <- renderPlot({
    scDRgene(sc2conf, sc2meta, input$sc2a3drX2, input$sc2a3drY2, input$sc2a3inp2,
             input$sc2a3sub1, input$sc2a3sub2,
             "sc2gexpr.h5", sc2gene,
  ...
  output$sc2a3oup2.pdf <- downloadHandler(
    ...
    content = function(file) { ggsave(
      ...
      plot = scDRgene(sc2conf, sc2meta, input$sc2a3drX2, input$sc2a3drY2, input$sc2a3inp2,
                      input$sc2a3sub1, input$sc2a3sub2,
                      "sc2gexpr.h5", sc2gene,
                      input$sc2a3siz, input$sc2a3col2, input$sc2a3ord2,
                      input$sc2a3fsz, input$sc2a3asp, input$sc2a3txt) )
